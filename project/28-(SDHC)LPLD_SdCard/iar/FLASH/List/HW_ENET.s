...
//    Locale       =  C
//    List file    =  
//        G:\AIcar\@@\xiugai\project\28-(SDHC)LPLD_SdCard\iar\FLASH\List\HW_ENET.s
//
///////////////////////////////////////////////////////////////////////////////
...
//  533   ENET->PALR = (uint32)((pa[0]<<24) | (pa[1]<<16) | (pa[2]<<8) | pa[3]);
        ...
        LDRB     R0,[R4, #+3]
        ORRS     R1,R0,R1
        LDR.N    R0,??DataTable14_48  ;; 0x400c00e4
        STR      R1,[R0, #+0]
//  534   ENET->PAUR = (uint32)((pa[4]<<24) | (pa[5]<<16));
        ...
        LSLS     R1,R1,#+16
        ORRS     R1,R1,R0, LSL #+24
        LDR.N    R0,??DataTable14_49  ;; 0x400c00e8
        STR      R1,[R0, #+0]
//  535   
...
//  539     ENET->IAUR |= (uint32)(1 << (crc - 32));
        LDR.N    R2,??DataTable14_26  ;; 0x400c0118
        LDR      R3,[R2, #+0]
        MOVS     R5,#+1
        SUBS     R1,R0,#+32
        LSLS     R5,R5,R1
...
//  596             | ENET_MMFR_DATA(data);
        ...
        UXTH     R0,R0            ;; ZeroExt  R0,R0,#+16,#+16
        ORRS     R4,R0,R4
        LDR.N    R0,??DataTable14_51  ;; 0x50020000
        ORRS     R4,R0,R4
...
//  641             | ENET_MMFR_TA(0x02);
        LDR.N    R7,??DataTable14_52  ;; 0x400c0040
        MOVS     R4,R3
        UXTH     R4,R4            ;; ZeroExt  R4,R4,#+16,#+16
        LSLS     R4,R4,#+23
        ANDS     R4,R4,#0xF800000
        ...
        ANDS     R0,R0,#0x7C0000
        ORRS     R4,R0,R4
        LDR.N    R0,??DataTable14_54  ;; 0x60020000
        ORRS     R4,R0,R4
        STR      R4,[R7, #+0]
//  642   
//  643   //等待MII传输完成中断事件
