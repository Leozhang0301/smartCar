...
//    Locale       =  C
//    List file    =  
//        G:\AIcar\@@\xiugai\project\28-(SDHC)LPLD_SdCard\iar\FLASH\List\HW_SDHC.s
//
///////////////////////////////////////////////////////////////////////////////
...
??LPLD_SDHC_SetBaudrate_5:
        LDR.W    R7,??DataTable8_8  ;; 0x400b1024
        LDR      R7,[R7, #+0]
        LSLS     R7,R7,#+28
...
//  105   SDHC->IRQSTAT |= SDHC_IRQSTAT_DTOE_MASK;
        LDR.W    R7,??DataTable9  ;; 0x400b1030
        LDR      R12,[R7, #+0]
...
//  168   SDHC->SYSCTL = SDHC_SYSCTL_RSTA_MASK | SDHC_SYSCTL_SDCLKFS(0x80);
        LDR.W    R7,??DataTable8_7  ;; 0x400b102c
...
??LPLD_SDHC_Init_1:
        LDR.W    R8,??DataTable8_8  ;; 0x400b1024
        LDR      R0,[R8, #+0]
        TST      R0,#0x3
...
//  193         | SDHC_IRQSTATEN_TCSEN_MASK | SDHC_IRQSTATEN_CCSEN_MASK;
        ...
        LDR.W    R2,??DataTable10  ;; 0x400b1034
        STR      R1,[R2, #+0]
//  194   
//  195   //等待80个初始时钟
...
//  249       blkattr = SDHC_BLKATTR_BLKSIZE(command->BLOCKSIZE) | SDHC_BLKATTR_BLKCNT(0xFFFF);
        LDR      R6,[R4, #+12]
        LSLS     R6,R6,#+19       ;; ZeroExtS R6,R6,#+19,#+19
        LSRS     R6,R6,#+19
...
//  270   SDHC->CMDARG = command->ARGUMENT;
        LDR      R0,[R4, #+4]
        LDR.W    R1,??DataTable10_1  ;; 0x400b1008
        STR      R0,[R1, #+0]
...
//  272   SDHC->DSADDR = 0;
        MOVS     R0,#+0
        LDR.W    R1,??DataTable10_2  ;; 0x400b1000
        STR      R0,[R1, #+0]
//  273   
//  274   //发送命令
//  275   SDHC->XFERTYP = xfertyp;
        LDR.W    R0,??DataTable10_3  ;; 0x400b100c
        STR      R5,[R0, #+0]
//  276   
//  277   //等待响应
//  278   if (LPLD_SDHC_WaitStatus (SDHC_IRQSTAT_CIE_MASK | SDHC_IRQSTAT_CEBE_MASK | SDHC_IRQSTAT_CCE_MASK | SDHC_IRQSTAT_CC_MASK) != SDHC_IRQSTAT_CC_MASK)
        LDR.W    R0,??DataTable10_4  ;; 0xe0001
...
//  300     command->RESPONSE[0] = SDHC->CMDRSP[0];
        LDR.W    R0,??DataTable10_5  ;; 0x400b1010
        LDR      R0,[R0, #+0]
        STR      R0,[R4, #+16]
...
//  303       command->RESPONSE[1] = SDHC->CMDRSP[1];
        LDR.W    R0,??DataTable10_6  ;; 0x400b1014
        LDR      R0,[R0, #+0]
        STR      R0,[R4, #+20]
//  304       command->RESPONSE[2] = SDHC->CMDRSP[2];
        LDR.W    R0,??DataTable10_7  ;; 0x400b1018
        LDR      R0,[R0, #+0]
        STR      R0,[R4, #+24]
//  305       command->RESPONSE[3] = SDHC->CMDRSP[3];
        LDR.W    R0,??DataTable10_8  ;; 0x400b101c
        LDR      R0,[R0, #+0]
        STR      R0,[R4, #+28]
//  306     }
//  307   }
...
??LPLD_SDHC_IOC_0:
        ...
        LDR.W    R1,??DataTable10_10  ;; 0x61a80
        LDR      R0,[R9, #+0]
        MUL      R0,R8,R0
        BL       LPLD_SDHC_Init
        MOVS     R7,R0
...
??LPLD_SDHC_IOC_11:
        LDR.W    R0,??DataTable10_11  ;; 0x81a0000
        STR      R0,[SP, #+8]
...
??LPLD_SDHC_IOC_13:
        LDR.W    R10,??DataTable11  ;; 0x5020000
        STR      R10,[SP, #+8]
...
//  431       command.COMMAND = ESDHC_CMD55;
        LDR.W    R10,??DataTable11_1  ;; 0x371a0000
        STR      R10,[SP, #+8]
...
//  460         command.COMMAND = ESDHC_CMD39;
        LDR.W    R10,??DataTable11_3  ;; 0x27020000
        STR      R10,[SP, #+8]
...
??LPLD_SDHC_IOC_25:
        LDR.W    R11,??DataTable11_4  ;; 0x29020000
        STR      R11,[SP, #+8]
